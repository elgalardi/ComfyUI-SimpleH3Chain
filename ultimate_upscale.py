# Adapted from PlagueKind/ComfyUI-PlagueKind-Nodes (MIT).
# See licenses/PlagueKind-UltimateUpscale-MIT.txt. H3-only fork; optional explicit unloads.
"""SimpleH3 Ultimate Upscale - one node for the full latent re-enhancement loop.
...
def resolve_grid_on_frame(sp, w, h, t, ol_w, ol_h, min_tile):
    ...
    if mode == "auto":
        ...
        print("[SimpleH3] auto tiling on the real frame %dx%dpx (%d video tokens): "
        ...
        print("[SimpleH3] rows_cols solved on the real frame %dx%dpx: %dx%d grid, "
...
def load_upscale_model(name, device, precision):
    ...
    print(f"[SimpleH3-UltimateUpscale] Loaded upscale model: {name}")
    return model
...
def spatial_process_joint(chunk_v, chunk_a, cond, sp, model, noise, sampler,
    ...
    print("[SimpleH3] joint tiling: %dx%d = %d tiles, %d step(s), stepped together"
...
def spatial_process(chunk_v, chunk_a, cond, sp, model, noise, sampler, sigmas, negative, cfg):
    ...
    try:
        ...
        print("[SimpleH3] shared tile noise unavailable (%s) - falling back to "
...
class SimpleH3LatentUpscaleWithModelParams(io.ComfyNode):
    """Bundle the H3 3D model-based latent upscale settings consumed by the Ultimate Upscale node."""

    @classmethod
    def define_schema(cls):
        return io.Schema(
            node_id="SimpleH3LatentUpscaleWithModelParams",
            display_name="SimpleH3 Latent Upscale with Model Params",
            category="Simple H3 Chain/Upscale",
            description=(
                "Bundle the H3 3D latent upscale settings for the 'SimpleH3 Ultimate "
            ...
            outputs=[
                H3_UPSCALE_PARAM.Output("latent_upscale_param",
                                        tooltip="Upscale settings consumed by 'SimpleH3 Ultimate Upscale'."),
            ],
        )
...
class SimpleH3LatentUpscaleParams(io.ComfyNode):
    ...
    def define_schema(cls):
        return io.Schema(
            node_id="SimpleH3LatentUpscaleParams",
            display_name="SimpleH3 Latent Upscale Params",
            category="Simple H3 Chain/Upscale",
            description=(
                "Bundle model-free latent upscale settings for the 'SimpleH3 Ultimate "
            ...
            outputs=[
                H3_UPSCALE_PARAM.Output("latent_upscale_param",
                                        tooltip="Model-free upscale settings consumed by 'SimpleH3 Ultimate Upscale'."),
            ],
        )
...
class SimpleH3TemporalSplitParams(io.ComfyNode):
    """Bundle the temporal split settings consumed by the Ultimate Upscale node."""

    @classmethod
    def define_schema(cls):
        return io.Schema(
            node_id="SimpleH3TemporalSplitParams",
            display_name="SimpleH3 Temporal Split Params",
            category="Simple H3 Chain/Upscale",
            description=(
                "Bundle the temporal split settings for the 'SimpleH3 Ultimate Upscale' "
            ...
            outputs=[
                H3_TEMPORAL_PARAM.Output("temporal_split_param",
                                         tooltip="Temporal split settings consumed by 'SimpleH3 Ultimate Upscale'."),
            ],
        )
...
class SimpleH3SpatialSplitParams(io.ComfyNode):
    """Bundle the spatial split settings consumed by the Ultimate Upscale node."""

    @classmethod
    def define_schema(cls):
        return io.Schema(
            node_id="SimpleH3SpatialSplitParams",
            display_name="SimpleH3 Spatial Split Params",
            category="Simple H3 Chain/Upscale",
            description=(
                "Bundle the spatial tile settings for the 'SimpleH3 Ultimate Upscale' "
            ...
            outputs=[
                H3_SPATIAL_PARAM.Output("spatial_split_param",
                                        tooltip="Spatial split settings consumed by 'SimpleH3 Ultimate Upscale'."),
    ...
    def execute(cls, upscale_width, upscale_height, tile_size_mode, tile_width,
        ...
        if tile_size_mode == "auto":
            ...
            print(f"[SimpleH3 Spatial Split Params] auto mode: grid solved at "
        ...
        if tile_size_mode == "rows_cols":
            ...
            print(f"[SimpleH3 Spatial Split Params] rows_cols mode: {grid_rows}x{grid_cols} "
...
class SimpleH3UltimateUpscale(io.ComfyNode):
    """One node for the full latent re-enhancement pipeline."""

    @classmethod
    def define_schema(cls):
        return io.Schema(
            node_id="SimpleH3UltimateUpscale",
            display_name="Simple H3 Ultimate Upscale",
            category="Simple H3 Chain/Upscale",
            description=(
            ...
            inputs=[
                ...
                io.Boolean.Input("keep_model_loaded", default=True,
                                 tooltip="Skip this node\'s explicit H3 unloads. ComfyUI may still offload for VRAM pressure or VAE decoding. Turn off if memory is insufficient."),
                ...
                H3_UPSCALE_PARAM.Input("latent_upscale_param", optional=True,
                                       tooltip="Output of 'SimpleH3 Latent Upscale with Model Params' (H3 3D upscaler) OR 'SimpleH3 Latent Upscale Params' (model-free interpolation). Leave unconnected to skip upscaling."),
    ...
    def execute(cls, latent, conditioning, model, noise, sampler, sigmas,
        ...
        if video.shape[0] != 1:
            raise ValueError("SimpleH3UltimateUpscale expects a single-video latent (batch 1)")
        ...
        for i, (k0, f0, k1, f1) in enumerate(bounds):
            ...
            if latent_upscale_param is not None:
                ...
                if not keep_model_loaded and use_model and str(latent_upscale_param["device"]) == "cuda" and hasattr(model, "clone_base_uuid"):
        ...
        if not keep_model_loaded and hasattr(model, "clone_base_uuid"):
            comfy.model_management.unload_model_and_clones(model, unload_additional_models=False)
            comfy.model_management.soft_empty_cache()
